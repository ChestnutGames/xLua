...
struct vars *
lbind_new() {
	struct vars *v = (struct vars *)malloc(sizeof(*v));
	if (v == NULL)
		return NULL;
	v->n = 0;
	v->cap = 0;
...
newvalue(struct vars *v) {
	if (v->n >= v->cap) {
		...
		struct lvar * nv = (struct lvar *)malloc(cap * sizeof(*nv));
		if (nv == NULL)
			return NULL;
...
lbind_args(lua_State *L) {
	...
	args = (struct vars *)lua_touserdata(L, -1);
	lua_pop(L, 1);
	return args;
}
...
lcall(lua_State *L) {
	...
	struct vars *args = (struct vars *)lua_touserdata(L,2);
	lua_settop(L, 0);
...
lbind_call(lua_State *L, const char * funcname, struct vars *args) {
	...
	if (ret == LUA_OK) {
		struct vars * result = (struct vars *)lua_touserdata(L,-1);
		lua_pop(L, 1);
		return result;
	} else {
