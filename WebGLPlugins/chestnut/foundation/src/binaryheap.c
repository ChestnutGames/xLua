 ...
 *
 * 
 * binaryheap.c is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Lesser General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * binaryheap.c is distributed in the hope that it will be useful,
...
void binary_heap_new(binary_heap_t** out, compare_f cmp)
{
    ...
    heap->data = (void **)BINARY_HEAP_ALLOC(BINARY_HEAP_INITIAL_CAPACITY * sizeof(void*));
...
void    binary_heap_search(binary_heap_t* heap, void** out, void *elt, compare_f cmp) {
	assert(heap);
	int i = 0;
	for (; i < heap->capacity; i++)
	{
		void *ptr = heap->data[i];
		if (cmp(ptr, elt) == 0)
		{
			*out = ptr;
			return;
